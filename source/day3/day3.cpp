...
void Day3::solve2(std::string input)
{
	auto tokens = split(input, "\n");
	std::vector<pairOfpartNumbersAndTheirCoordinatesType> partNumbersAndTheirCoordinates;
	...
	for (auto y = 0; y < LENGTH_Y; y++)
	{
		...
		for (auto x = 0; x < LENGTH_X; x++)
		{
			...
			std::unordered_map<int, pairOfCoordinatesAndWhetherTheyAppeared> adjacentParts;
			...
			{
				std::vector<int> _adjacentParts;
				for (const auto& part : adjacentParts)
				{
					_adjacentParts.push_back(part.first);
				}
				printf("part numbers %d and %d\n", _adjacentParts[0], _adjacentParts[1]);
				sum += _adjacentParts[0] * _adjacentParts[1];
			}
		}
	}
